...
Source: claude-plugin-mcp/src/ingestion/reply_attachment.py (verbatim).
See src/redline/lib/__init__.py for the package-level upgrade warning.

Note (10P): unexercised by 10P-prep input fixture (no comments
present). Port retained for fidelity and future inputs that DO
have comments.
"""
...
def attach_reply_comments(
    ...
    commentsExtended paraIdParent chain and nests them as
    TrackedChangeEntry objects in the parent entry's replies list.
