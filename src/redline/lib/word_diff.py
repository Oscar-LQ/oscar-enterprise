...
Tokenizes text with r'\\S+|\\s+' so punctuation stays attached to
words (legal standard: "claims." is one token, not "claims" + ".").
Whitespace sequences become separate tokens. Tokens are encoded to
unique Unicode characters, diffed via diff_match_patch, then decoded
back to produce word-granularity segments.

Source: claude-plugin-mcp/src/pipeline/word_diff.py (verbatim).
...
def _encode_tokens(
    ...
    def encode(tokens: list[str]) -> str:
        """Encode a list of text tokens into single-character representations."""
        nonlocal next_code
        chars: list[str] = []
        for token in tokens:
