"""OOXML helpers for constructing comment XML in .docx documents.

Despite the filename (kept from MCP for traceability), this module
holds the general OPC plumbing for any comment operation, not just
threaded replies. Both add_comment and reply paths import from here.

Functions:
- get_or_create_comments_part / get_or_create_comments_extended_part
- get_next_comment_id, allocate_para_id, collect_existing_para_ids
- add_reply_comment + anchor_reply_to_parent_range (for threaded replies)

Source: claude-plugin-mcp/src/negotiation/reply_helpers.py (verbatim,
import path adapted for the lazy comment_ids_helpers reference).
