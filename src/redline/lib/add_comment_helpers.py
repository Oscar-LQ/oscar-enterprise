...
Also provides standalone comment creation in comments.xml and
commentsExtended.xml, similar to add_reply_comment but WITHOUT
paraIdParent (no thread parent).

Source:
- anchor_comment_to_tracked_change, anchor_comment_to_text,
  create_standalone_comment, _get_content_children,
  _find_paragraphs_containing_text, _get_paragraph_run_text,
  _insert_range_markers:
  claude-plugin-mcp/src/negotiation/add_comment_helpers.py (verbatim,
  imports adapted for redline.lib paths)
- CommentError: claude-plugin-mcp/src/models/comment.py (folded in)
...
from src.redline.lib.counter_propose_helpers import find_tracked_change_element
from src.redline.lib.reply_helpers import W15_NS


class CommentError(Exception):
    """Raised when comment anchoring or creation fails.

    Source: claude-plugin-mcp/src/models/comment.py (folded in).
    """
...
def anchor_comment_to_tracked_change(
    ...
    Finds the w:ins or w:del by ooxml_id, then inserts commentRangeStart
    before it in the parent paragraph, commentRangeEnd after it, and a
    commentReference run after the range end.
    """
...
def anchor_comment_to_text(
    ...
    if not anchors:
        raise CommentError(
            f"No content elements found in paragraph matching {target_text!r}"
        )

    cid_str = str(comment_id)
    _insert_range_markers(
        paragraph, anchors[0], cid_str, end_element=anchors[-1],
    )
