"""Sprint 10O — driver for the planner-executor head-to-head pipeline.
...
Usage:
    python src/redline/experiments/sprint-10O/run.py
"""
...
def _topological_sort(plan: list[dict]) -> list[dict]:
    ...
    for p in plan:
        for d in p.get("depends_on") or []:
            if d not in by_id:
                print(f"[10O] depends_on: instruction {p['id']!r} references missing id {d!r}; ignoring")
                continue
            in_degree[p["id"]] += 1
...
def _write(path: Path, text: str) -> None:
    ...
    print(f"[10O] wrote {path.name} ({len(text)} chars)")
...
def run_once() -> int:
    ...
    print(
        f"[10O] planner={p_provider}/{p_model} | executor={e_provider}/{e_model} | "
    ...
    _append_transcript(
        transcript,
        f"\n=== Sprint 10O Phase 3 | planner={p_provider}/{p_model} | "
    ...
    print(f"[10O] nda-input.docx = {nda_input.stat().st_size} bytes")

    # 2. pipeline.prepare.
    docx_bytes = nda_input.read_bytes()
    contract_text = pipeline.prepare(docx_bytes, clean_view=False, author="Oscar")
    print(f"[10O] contract text = {len(contract_text)} chars (with header)")
    ...
    print(f"[10O] invoking planner {p_provider}/{p_model} ...")
    ...
    print(f"[10O] planner reply = {len(p_raw)} chars")

    # 5. Parse plan.
    try:
        parsed_plan = parse_plan_response(p_raw)
    except ValueError as exc:
        print(f"[10O] PLANNER PARSE FAILED: {exc}")
    ...
    print(
        f"[10O] plan: parse_method={plan_method} | instructions={len(plan)} "
    ...
    if not plan:
        print("[10O] empty plan — nothing to execute")
    ...
    try:
        ...
        print(f"[10O] TOPO SORT FAILED: {exc}")
    ...
    for idx, instruction in enumerate(sorted_plan, start=1):
        ...
        print(f"[10O] invoking executor for instruction {pid} ({idx}/{len(sorted_plan)}) ...")
        ...
        try:
            ...
            print(f"[10O] EXECUTOR {pid} PARSE FAILED: {exc}")
        ...
        if not target or not new:
            print(f"[10O] EXECUTOR {pid}: empty target_text or new_text — skipping")
    ...
    print(
        f"[10O] executor pass complete: {len(edits)} edits, "
    ...
    if not edits_for_pipeline:
        print("[10O] no edits to apply — Outcome C")
    ...
    print(
        f"[10O] nda-output.docx = {output_path.stat().st_size} bytes | "
        f"applied={result['applied']} | skipped={result['skipped']}"
    )

    # 9. Verify.
    ok, notes = verify_output(output_path)
    print(f"[10O] verify_output ok={ok}")
    for line in notes:
        print(f"  {line}")
    ...
    print(f"[10O] done in {elapsed:.1f}s | mechanical_ok={ok}")
