...
================================================================
UPGRADE-BRITTLENESS WARNING (added Sprint 10O — flagged for 10P+)
================================================================

This pipeline imports private Adeu surfaces:
  - engine._create_track_change_tag        (private method)
  - mapper._build_map                      (private method)
  - mapper.spans                           (via PlainTextIndex)
  - adeu.ingest._extract_blocks            (underscore-prefixed)
  - adeu.redline.comments.CommentsManager  (internal module)
  - adeu.redline.mapper.DocumentMapper     (internal module)
  - adeu.utils.docx.create_element / iter_document_parts

Adeu's PUBLIC API is `RedlineEngine.process_batch` plus the
discriminated-union of edit types (ModifyText / AcceptChange /
RejectChange / ReplyComment). Adeu has been moving at roughly one
minor release per fortnight (1.0 → 1.3.3 in two months from
February to April 2026). Each minor or patch release MAY break
private-surface internals without warning — they are not part of
the version-compatibility contract.

A future sprint (10P or later) should refactor the inline word-diff
path onto `process_batch` to (a) reduce upgrade brittleness, and
(b) recover comment-attachment semantics on inline edits (the
current inline path bypasses Adeu's comment integration; Q1 of the
10N addendum identified comment-drop as Vibe-architecture
inheritance from this same path).

Do not undertake that refactor inside 10O — explicitly out of
scope. 10O carries this pipeline forward unchanged from 10N
(which carried it from 10M's Vibe port). The flag stays here
until 10P+ resolves it.
================================================================
"""
...
def prepare(docx_bytes: bytes, clean_view: bool = False, author: str = "Zenith Counsel") -> str:
    """
...
def apply_edits(edits_json: str, fallback_bytes: bytes = None, polish_formatting: bool = False) -> dict:
    ...
    if _engine is None:
        ...
        try:
            _engine = RedlineEngine(stream, author="Zenith Counsel")
        finally:
            stream.close()
    ...
    edits = [
        ModifyText(target_text=e.get("target_text", ""), new_text=e.get("new_text", ""))
        for e in edits_data
    ]
    ...
    if polish_formatting:
        try:
            ...
            if original:
                ref_styler = Styler(original, author="Zenith Counsel")
                ref_formats = ref_styler.detect_reference_formats()

            # Run Styler on REDLINED document with original reference
            styler = Styler(doc_bytes, author="Zenith Counsel", original_reference=ref_formats)
            result = styler.run()
...
def _normalize_edit_whitespace(edit):
    ...
    return ModifyText(target_text=target, new_text=new)
...
def _delegate_with_match(engine, edit, mapper, start_idx):
    ...
    Creates a proxy ModifyText with _match_start_index set so the engine
    ...
    on ModifyText (models.py:44-46). Check if these fields still exist.
    """
    proxy = ModifyText(target_text=edit.target_text, new_text=edit.new_text)
    ...
    if runs_plain_text != first_line:
        ...
        if reconstructed != first_line:
            if not has_newlines:
                ...
                proxy = ModifyText(target_text=edit.target_text, new_text=clean_new_text)
