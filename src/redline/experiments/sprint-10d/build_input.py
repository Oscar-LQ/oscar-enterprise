"""Sprint 10D — build the synthetic NDA input document.

Generates ``nda-input.docx`` from a Python-defined clause structure that
mirrors ``nda-input.md``. The markdown file is the human-legible source;
this module is the canonical source for the ``.docx``. When the NDA is
edited, update both files together — they must stay in sync.

Run directly to (re)generate the document::

    python src/redline/experiments/sprint-10d/build_input.py
"""
...
# Each clause is (heading, body). Bodies are single-paragraph; any multi-sentence
# body stays in one paragraph to keep redline matching simple.
