"""Sprint 10M — Vibe Legal Redliner response parser, ported verbatim.

Ports `parseAIResponse`, `validateEdits`, `tryParseJSON`,
`repairTruncatedJSON`, and `rescueEdits` from
`src/utils/ai-bundle.js:414-570` to Python.
...
Reports which layer succeeded via the `parse_method` field on the
returned dict (diagnostic per Arturs's 10M brief addition).

Faithful port — no behaviour changes.
"""
...
def validate_edits(parsed: Any) -> dict[str, Any]:
    ...
    """
    edits_raw = parsed.get("edits") if isinstance(parsed, dict) else None
    if not isinstance(edits_raw, list):
        return {
            "edits": [],
            "summary": "Invalid response format - no edits array",
            "reasoning": None,
        }
