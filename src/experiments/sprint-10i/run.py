"""Sprint 10I — Executioner-framing capability test.

Single-agent MiniMax run. One clause-9 executioner, direct invocation, no GC,
no HOC, no planner/executor split, no sub-agents declared. Tests whether
MiniMax-M2.7 can decompose a litigation→arbitration transformation into
2-4 narrow Adeu edits when given executioner framing (pre-decided goal,
scope fixed to §9, no document-level reasoning required).

Research lineage:
  * 10E handed exact spans (execution only) — worked.
  * 10F/10G used document-level single-agent framing — bundled one wide edit.
  * 10H planner+executor split — confounded by HOC dropping clause text.
  * 10I tests executioner framing directly, single-agent, no orchestration.

Scope is deliberately narrow: capability test, not architecture test. If
MiniMax can function as a clause-level executioner under this framing,
10J designs the planner + multiple-executioners architecture. If it still
bundles, 10J explores a deterministic word-diff executor (Shape B).

Single attempt; no iteration. See plan at
/sandbox/.claude/plans/zany-drifting-globe.md and the sprint brief.

Input: ``nda-input.docx`` regenerated by ``build_input.py`` (byte-comparable
to 10E/10F/10G/10H). Output: ``nda-output.docx`` in this directory;
``tool-calls.jsonl`` captures the specialist's verbatim tool calls;
``transcript.txt`` records the full system prompt, invocation message,
message trace, tool calls, verify_output notes, and clean-view §9 read-back.
"""
