...
9. **Decomposition ceiling now sits at Stage 1 (drafter output shape) — 10J Outcome B refines the architectural read.** 10F showed MiniMax identifies clause-level but cannot decompose span-level under document framing. 10G falsified the planning-absence hypothesis. 10H was confounded by HOC text-drop. 10I-Sonnet bundled wider than any prior MiniMax run (71-word `w:ins`) — ceiling is prompt-framing, not model-tier. 10J built Shape B (LLM drafts + Python word-diff + Adeu applies) and measured it end-to-end on the same transformation. **Finding: pipeline mechanics are sound (Stage 1 JSON clean, Stage 2 diff/block-group correct, Stage 3 Adeu apply atomic) but the single edit produced is still lawyer-shape-fail (`w:ins`=54 words OVER-BROAD, `w:del`=29 words SUSPICIOUS)**. Without conservation guidance in the Stage 1 prompt (dropped deliberately to keep the hypothesis test clean — confirmed with Arturs at plan time, Plan-agent argument aligned), MiniMax's natural drafting style rewrites the dispute-resolution sentence wholesale — so the word-diff has only one logical block to emit. **The bundling moved upstream from the decomposition step to the drafting step.** Re-characterisation: what looked like "LLM decomposition ceiling" is better read as **"LLM naturally produces prose-level rewrites when given a substance change, regardless of framing"** — drafting or editing are the same cognitive task, and the output shape depends on whether the prompt asks for conservation. Open question for 10K: does a conservation-discipline sentence in Stage 1 produce the 2–5 narrow blocks the diff substrate needs? If yes, Shape B works with a conservative drafter — the combination is the production substrate. If no (MiniMax ignores conservation as 10G's plan-first discipline was ignored), the substrate does not rescue us from model shape, and 10L considers element-level scaffolding or CPM-style delegation as alternatives. Shape B is NOT off the table; it's waiting on one prompt iteration. The LLM+CODE pipeline was not proven sufficient by 10J; it was proven mechanically sound, with the remaining question at Stage 1 scope.
   - Trigger: **reached — 10K tests conservation-discipline prompt iteration on the Shape B pipeline**.
   - Source: Sprint 10E scope boundary; Sprint 10F §10G proposal; Sprint 10G architectural-judgement section; Sprint 10H control-run evidence; Sprint 10I primary outcome D; Sprint 10I Sonnet reference outcome C; **Sprint 10J outcome B (pipeline mechanically sound; drafter output shape is the ceiling)**.
