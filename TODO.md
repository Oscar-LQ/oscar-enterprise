...
9. **Identification vs. execution — design space collapsed to LLM+CODE executor by 10I.** 10F showed MiniMax can identify clause-level but cannot decompose span-level under document framing. 10G falsified the planning-absence hypothesis. 10H was confounded by HOC text-drop before the planner capability could be measured. 10I tested executioner framing directly: **MiniMax primary (outcome D — framework-blocked) gave no decomposition data; Sonnet reference (outcome C) bundled the transformation into one 71-word `modify_text` — wider than any prior MiniMax run under any framing.** Combined reading: **the decomposition ceiling under count-and-shape prompt guidance is prompt-framing, not model-tier. Both single-specialist-on-frontier AND all-MiniMax-clause-executioner are ruled out** — the first on bundling (new 10I-Sonnet evidence), the second on bundling plus framework-tool-leak (10I primary). The **2-LLM split (10H Shape A)** remains open only to the extent the executor can be driven to narrow edits via plan scaffolding — 10H's control run confirmed plan-in → byte-identical-tool-call-out works, but the LLM-on-LLM split does not magically unlock decomposition if the underlying ceiling is LLM decomposition itself. **LLM+CODE executor (Shape B — word-diff pipeline port from Claude-Plugin-MCP) is now the single live candidate for surgical-span output.** Trigger moves from "if 10I fails" to "10J scope — immediate".
   - Trigger: **reached — 10J is Shape B**.
   - Source: Sprint 10E scope boundary; Sprint 10F §10G proposal; Sprint 10G architectural-judgement section; Sprint 10H control-run evidence; Sprint 10I primary outcome D; Sprint 10I Sonnet reference outcome C.
