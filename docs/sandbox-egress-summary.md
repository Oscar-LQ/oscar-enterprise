...
| `minimax` | `api.minimax.io:443` | `/sandbox/.venv/bin/python(3)`, `/app/.venv/bin/python(3)`, `/sandbox/.uv/python/**`, `/usr/bin/curl` | MiniMax LLM HTTP API (OpenAI-compatible `/v1/chat/completions`). Used by `src/llm/minimax.py`. | Mixed — SIT keeps it if Oscar's runtime calls MiniMax directly (not via OpenRouter) | — |
