...
| `OSCAR_LLM_REDLINE_SPECIALIST_PROVIDER` | Per-role DI slot (ADR 010): provider for the redline specialist (document-level Adeu-driven redlining). Read by `get_chat_model(env_prefix="OSCAR_LLM_REDLINE_SPECIALIST")`. Orphaned as of Sprint 10H (replaced by `OSCAR_LLM_REDLINE_EXECUTOR_*`); kept in the inventory until 10I confirms the split architecture. | Sprint 10D redline-specialist. | Sprint 10D | Sprint 10H (orphaned) |
| `OSCAR_LLM_REDLINE_SPECIALIST_MODEL` | Per-role DI slot: specialist model id for redlining (Sprint 10D: `MiniMax-M2.7`). Orphaned as of Sprint 10H. | Sprint 10D redline-specialist. | Sprint 10D | Sprint 10H (orphaned) |
| `OSCAR_LLM_REDLINE_SPECIALIST_API_KEY` | Per-role DI slot: API key for the redline specialist's provider. Orphaned as of Sprint 10H. | Sprint 10D redline-specialist. | Sprint 10D | Sprint 10H (orphaned) |
| `OSCAR_LLM_REDLINE_PLANNER_PROVIDER` | Per-role DI slot (ADR 010): provider for the redline planner (Sprint 10H Shape A — decomposes a requested redline into a narrow JSON edit plan; makes no tool calls). Frontier-tier — typically `openrouter`. Read by `get_chat_model(env_prefix="OSCAR_LLM_REDLINE_PLANNER")`. | Sprint 10H redline-planner. | Sprint 10H | Sprint 10H |
| `OSCAR_LLM_REDLINE_PLANNER_MODEL` | Per-role DI slot: model id for the redline planner (Sprint 10H: `openai/gpt-5.4`). | Sprint 10H redline-planner. | Sprint 10H | Sprint 10H |
| `OSCAR_LLM_REDLINE_PLANNER_API_KEY` | Per-role DI slot: API key for the redline planner's provider. Typically an OpenRouter key distinct from the MiniMax specialist-tier keys. | Sprint 10H redline-planner. | Sprint 10H | Sprint 10H |
| `OSCAR_LLM_REDLINE_EXECUTOR_PROVIDER` | Per-role DI slot (ADR 010): provider for the redline executor (Sprint 10H Shape A — consumes the planner's plan and applies each entry via modify_text/insert_text). Specialist-tier; typically `minimax`. Read by `get_chat_model(env_prefix="OSCAR_LLM_REDLINE_EXECUTOR")`. Rename of Sprint 10D's redline-specialist to reflect the narrowed role. | Sprint 10H redline-executor. | Sprint 10H | Sprint 10H |
| `OSCAR_LLM_REDLINE_EXECUTOR_MODEL` | Per-role DI slot: specialist model id for the redline executor (Sprint 10H: `MiniMax-M2.7`). | Sprint 10H redline-executor. | Sprint 10H | Sprint 10H |
| `OSCAR_LLM_REDLINE_EXECUTOR_API_KEY` | Per-role DI slot: API key for the redline executor's provider. Typically matches the Head of Commercial key when both run on MiniMax. | Sprint 10H redline-executor. | Sprint 10H | Sprint 10H |
